...
int main (int argc, char *argv[])
{
  ...
  if (graphFile.empty () || traceFile.empty () || nodeNum <= 0 || duration <= 0 || logFile.empty () || animFile.empty() || voltage == 0 || capacity == 0 )
  {
      std::cout << "Usage of " << argv[0] << " :\n\n"
      "./waf --run \"mobilitytest"
      " --graphFile=randomgraph.xml"
      " --traceFile=src/mobility/examples/default.ns_movements"
      " --nodeNum=2 --duration=100.0 --logFile=mobility.log --animFile=mobility.xml --voltage=11.1 --capacity=1000\" \n\n"
      "NOTE: ns2-traces-file could be an absolute or relative path. You could use the file default.ns_movements\n"
  ...
  Ptr<UavRandomWaypointMobilityModel> model = CreateObject<UavRandomWaypointMobilityModel>(graphFile,energyModel);
  model->SetAttribute("Speed",StringValue("ns3::UniformRandomVariable[Min=0.0|Max=120.0]"));
  model->SetAttribute("PositionAllocator",PointerValue(taPositionAlloc));

  uav->AggregateObject(model);
...
//  model->printTrajectory();
  model->printCoverage(10,10);
